...
for i in range(len(Om)):
    for j in range(len(Ode)):
        ...
        popt, pcov = curve_fit(model, Eiso_bc, Epeak_bc, sigma=Epeak_err, p0=[0.5, 0.0], bounds=([0.0, -1.0], [1.0, 1.0]))
        a[i,j], b[i,j] = popt
...
masked_chi_surface = chi_surface #np.ma.array(chi_surface, mask=mask)
...
plt.scatter(Om_fit, Ode_fit, c='r', s=50, marker='x', label=f'Best fit: Om={Om_fit:.2f}, Ode={Ode_fit:.2f}')
plt.xlabel('Om')
plt.ylabel('Ode')
